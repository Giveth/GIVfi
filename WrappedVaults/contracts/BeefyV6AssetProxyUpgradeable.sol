...
contract BeefyV6AssetProxyUpgradeable is Initializable, WrappedPositionUpgradeable, Authorizable {
    ...
    /// @notice Initialize this contract and stores needed data
    ...
    function __BeefyV6AssetProxyUpgradeable_init(
        address vault_,
        address _token,
        string memory _name,
        ...
        IERC20Upgradeable(_token).approve(vault_, type(uint256).max);
        ...
        __WrappedPositionUpgradeable_init(_token, _name, _symbol);
        require(
            uint8(ERC20Upgradeable(_token).decimals()) == localVaultDecimals,
