...
contract DonationHandler is DonationHandlerRoles, ReentrancyGuard {
    ...
    function donateWithFee(address _token, address _recipient, uint256 _amount, uint256 _fee) external nonReentrant {
    ...
    function distribute(address[] calldata _token, address _to) external nonReentrant {
    ...
    event DonationRegistered(address indexed token, address indexed recipient, uint256 amount);
